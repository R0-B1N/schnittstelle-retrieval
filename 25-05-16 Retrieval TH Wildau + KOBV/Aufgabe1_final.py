...
#Quelle: Angepasste Abfrage aus Folie 16 und 17, Vorlesung vom 16.05.2025
...
def lese_datensaetze(http_antwort):
    ...
    try:
        ...
        return ""
    # evtl. print(f"DEBUG: Antwort in Wurzel geparsed")
...
def drucke_ergebnisse():
    ...
    with open(f"OAIexport_{timestamp}.json", "w", encoding = "utf-8") as f:
        # json Dump über Ergebnisvektor
        json.dump(Ergebnisse, f, ensure_ascii = False, indent = 2, sort_keys = False, separators = (",", ": "))
    print(f"OK: {len(Ergebnisse)} Datensätze gespeichert in OAIexport_{timestamp}.json")
...
try:
    if Funktionsweise == "1":
        ...
        # URL für Abfrage
        url = f"{BASE_URL}?verb=ListRecords&metadataPrefix=oai_dc&from={dt_from}"
        ...
        dt_until = input("Bitte Datum bis angeben: ").strip()
        try:
            dt.date.fromisoformat(dt_from)
            dt.date.fromisoformat(dt_until)
        except ValueError:
            print("Bitte Datum im Format YYYY-MM-DD eingeben.")
            exit()
        # URL für Abfrage
        url = f"{BASE_URL}?verb=ListRecords&metadataPrefix=oai_dc&from={dt_from}&until={dt_until}"
    ...
    # Request für Abfrage mit Fehlerbehandlung
    try:
...
# Ergebnisse ausgeben
# hier print
print(f"Keinen weiteren Resumption-Token erhalten, Ergebnis wird nun ausgegeben:")
drucke_ergebnisse()