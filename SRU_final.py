...
def Suche_SRU(Feld, Text, MaxErg):
    ...
    # Recherche zu Aufbau der Anfragen: https://wiki.k10plus.de/spaces/K10PLUS/pages/27361342/SRU
    if Feld == 'titel':
        url = f"http://sru.k10plus.de/opac-de-627?version=1.1&operation=searchRetrieve&recordSchema=marcxml&query=pica.tit%3D{Text}&maximumRecords={MaxErg}"
    elif Feld == 'autor':
        url = f"http://sru.k10plus.de/opac-de-627?version=1.1&operation=searchRetrieve&recordSchema=marcxml&query=pica.per%3D{Text}&maximumRecords={MaxErg}"
    elif Feld == 'isbn':
        url = f"http://sru.k10plus.de/opac-de-627?version=1.1&operation=searchRetrieve&recordSchema=marcxml&query=pica.isb%3D{Text}&maximumRecords={MaxErg}"
    else:
...
# Fehlerbehandlung falls Menge keine Zahl
if not Menge.isdigit():
    print("Die Menge muss eine Zahl sein.")
    exit()

try:
    Liste = Suche_SRU(Suchfeld, Suchbegriff, Menge)
    
    # mit Ergebnismatrix ausgabe generieren, falls Felder leer wird mit 'keine'Platzhalter aufgefüllt
    for i, eintrag in enumerate(Liste, start=1):
        print(f"\nErgebnis Nr. {i}:")
        print(f"Titel:       {eintrag['Titel'] if eintrag['Titel'] else 'keiner'}")
        print(f"Autor(en):   {'; '.join(eintrag['Autor']) if eintrag['Autor'] else 'keine'}")
        print(f"ISBN(s):     {'; '.join(eintrag['ISBN']) if eintrag['ISBN'] else 'keine'}")
        print(f"Schlagworte: {'; '.join(eintrag['Schlagworte']) if eintrag['Schlagworte'] else 'keine'}")
